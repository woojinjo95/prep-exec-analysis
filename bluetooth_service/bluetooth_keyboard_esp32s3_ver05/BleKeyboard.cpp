...
static const uint8_t _hidReportDescriptor[] = {
	...
	REPORT_COUNT(1), 0x01,		 //   REPORT  _COUNT (16)
	// USAGE_PAGE(1), 0x0C,		 //   USAGE_PAGE (Consumer)
	LOGICAL_MINIMUM(1), 0x00,	 //   LOGICAL_MINIMUM (0)
	LOGICAL_MAXIMUM(2), 0xFF, 0x07,	 //   LOGICAL_MAXIMUM (1)
	USAGE_MINIMUM(1), 0x00,	   //   USAGE_MINIMUM (0)
	USAGE_MAXIMUM(2), 0xFF, 0x07,	   //   USAGE_MAXIMUM (0x65)
...
{
  ESP_LOGD(LOG_TAG, "before init");
  delay_ms(10);
	BLEDevice::init("test");
	ESP_LOGD(LOG_TAG, "after init");

  ESP_LOGD(LOG_TAG, "before createServer");
	pServer = BLEDevice::createServer();
  ESP_LOGD(LOG_TAG, "after createServer");

  ESP_LOGD(LOG_TAG, "before setCallbacks");
	pServer->setCallbacks(this);
  ESP_LOGD(LOG_TAG, "after setCallbacks");

  ESP_LOGD(LOG_TAG, "before new BLEHIDDevice");
	hid = new BLEHIDDevice(pServer);
  ESP_LOGD(LOG_TAG, "after new BLEHIDDevice");

	inputKeyboard = hid->inputReport(KEYBOARD_ID); // <-- input REPORTID 
	...
	// hotfix : https://github.com/T-vK/ESP32-BLE-Keyboard/issues/152
...
void BleKeyboard::disconnect(void)
{
	...
	NIMBLEDevice::deleteBond(BLEDevice::getAddress());
#endif
}
...
{
	_mediaKeyReport[0] = (uint8_t)k[0];
	_mediaKeyReport[1] = (uint8_t)k[1];
...
{
	_mediaKeyReport[0] = (uint8_t)0;
	_mediaKeyReport[1] = (uint8_t)0;
