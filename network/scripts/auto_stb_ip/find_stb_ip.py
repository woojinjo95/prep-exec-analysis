    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            if (stb_ip is None or prev_state == EthernetState.down) and current_state == EthernetState.up:
                logger.info(f'New Settop nic conection detected! wait {STABLE_DELAY} seconds for stable connection')
                time.sleep(STABLE_DELAY)

                original_delay = get_value('hardware_configuration', 'packet_delay', db=RedisDBEnum.hardware)

                original_ip_values = {k: v for k, v in brute_ping_ipv4(private_ip, timeout=TIMEOUT)}
                traffic_change(nic=stb_nic, delay=original_delay + UNIT_DELAY)
                augmented_ip_values = {k: v for k, v in brute_ping_ipv4(private_ip, timeout=TIMEOUT)}
                traffic_change(nic=stb_nic, delay=original_delay)

                for ip, ping_value in augmented_ip_values.items()[::-1]:
