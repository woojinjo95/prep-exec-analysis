...
const AnalysisSummaryResultList: React.FC<AnalysisSummaryResultListProps> = ({ summary }) => {
  ...
  return (
    <div className="overflow-y-auto flex flex-col gap-y-1">
      {summary.freeze && <FreezeSummaryResultItem freeze={summary.freeze} />}
      {summary.loudness && <LoudnessSummaryResultItem loudness={summary.loudness} />}
      {summary.resume && <ResumeSummaryResultItem resume={summary.resume} />}
      {summary.boot && <BootSummaryResultItem boot={summary.boot} />}
      ...
      {summary.log_pattern_matching && (
        <LogPatternMatchingSummaryResultItem logPatternMatching={summary.log_pattern_matching} />
      )}
    </div>
  )
}
