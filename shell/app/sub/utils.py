...
def log(message: str, module: str):
    return {'timestamp': timestamp(), 'module': module, 'message': message}


def timestamp():
    return timestamp_to_datetime_with_timezone_str(time.time(), format="%Y-%m-%dT%H%M%SF%f%z", timezone='Asia/Seoul')
